...
async function seedData() {
  ...
  try {
    ...
    for (const p of practices) {
      const [existing]: any = await connection.query('SELECT id FROM yoga_practices WHERE name = ?', [p.name]);
      if (existing.length === 0) {
        await connection.query(
          `INSERT INTO yoga_practices (name, short_description, description, difficulty, duration, category, image) VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [p.name, p.short_description, p.description, p.difficulty, p.duration, p.category, p.image]
        );
      }
    }
    ...
    for (const ins of instructors) {
      const [existing]: any = await connection.query('SELECT id FROM instructors WHERE name = ?', [ins.name]);
      if (existing.length === 0) {
        await connection.query(
          `INSERT INTO instructors (name, bio, specialization, image) VALUES (?, ?, ?, ?)`,
          [ins.name, ins.bio, ins.specialization, ins.image]
        );
      }
    }
