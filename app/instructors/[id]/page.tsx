...
export default function InstructorProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <main className="min-h-screen bg-astrian-oat pt-32 pb-24 text-astrian-charcoal font-sans">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          
          {/* Back Button */}
          <Link href="/instructors" className="inline-flex items-center gap-2 text-astrian-sage font-semibold hover:text-astrian-moss transition-colors mb-10 group cursor-pointer">
          ...
          <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start mb-20">
            ...
            <div className="lg:col-span-5 relative aspect-[3/4] w-full rounded-[3rem] overflow-hidden border-[12px] border-white shadow-[0_24px_50px_rgba(17,24,39,0.04)] bg-astrian-cream">
              <Image
            ...
            <div className="lg:col-span-7 flex flex-col justify-center">
              <span className="text-sm font-semibold tracking-wider text-astrian-sage uppercase bg-astrian-sage/10 px-4 py-1.5 rounded-full self-start mb-4">
                {instructor.role}
              </span>
              <h1 className="text-4xl md:text-6xl font-bold font-display text-astrian-charcoal mb-6">
                {instructor.name}
              </h1>
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                {instructor.tags.map((tag, idx) => (
                  <span key={idx} className="text-xs font-semibold text-astrian-charcoal/70 bg-astrian-clay px-3 py-1.5 rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              {/* About Narrative */}
              <div className="text-lg text-astrian-charcoal/80 font-light leading-relaxed mb-8 space-y-4">
                <p>{instructor.about}</p>
              </div>

              {/* Teaching Philosophy Box */}
              <div className="glass-panel p-8 rounded-[2rem] border border-astrian-clay bg-white/50 relative overflow-hidden">
                <div className="absolute top-4 right-4 text-4xl text-astrian-sage/10 font-bold select-none">ॐ</div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-astrian-sage mb-2">Teaching Philosophy</h3>
                <p className="text-base text-astrian-charcoal italic font-light leading-relaxed">
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-20">
            <Card className="flex flex-col items-center text-center p-8 bg-white border border-astrian-clay/60 shadow-sm rounded-3xl">
              <span className="h-12 w-12 rounded-full bg-astrian-cream flex items-center justify-center text-astrian-sage mb-4 font-bold text-lg">★</span>
              <h4 className="text-sm font-bold text-astrian-charcoal/40 uppercase tracking-wider mb-1">Experience</h4>
              <p className="text-2xl font-bold text-astrian-charcoal font-display">{instructor.stats.experience}</p>
            </Card>

            <Card className="flex flex-col items-center text-center p-8 bg-white border border-astrian-clay/60 shadow-sm rounded-3xl">
              <Award className="h-6 w-6 text-astrian-sage mb-4" />
              <h4 className="text-sm font-bold text-astrian-charcoal/40 uppercase tracking-wider mb-1">Specializations</h4>
              <p className="text-xl font-bold text-astrian-charcoal font-display text-balance">{instructor.stats.specializations}</p>
            </Card>

            <Card className="flex flex-col items-center text-center p-8 bg-white border border-astrian-clay/60 shadow-sm rounded-3xl">
              <Languages className="h-6 w-6 text-astrian-sage mb-4" />
              <h4 className="text-sm font-bold text-astrian-charcoal/40 uppercase tracking-wider mb-1">Languages</h4>
              <p className="text-2xl font-bold text-astrian-charcoal font-display">{instructor.stats.languages}</p>
            </Card>
          </section>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 mb-20 items-start">
            {/* Left: Progress Bars */}
            <div>
              <h2 className="text-3xl font-bold text-astrian-charcoal mb-8 font-display">Expertise Focus</h2>
              <div className="space-y-6">
                {instructor.expertise.map((exp, idx) => (
                  <div key={idx} className="space-y-2">
                    <div className="flex justify-between text-sm font-medium text-astrian-charcoal">
                      <span>{exp.name}</span>
                      <span>{exp.value}%</span>
                    </div>
                    <div className="w-full bg-astrian-clay h-2.5 rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                        whileInView={{ width: `${exp.value}%` }}
                        viewport={{ once: true }}
                        transition={{ duration: 1, ease: "easeOut" }}
                        className="bg-astrian-sage h-full rounded-full"
                      />
                    </div>
                  </div>
                ))}
            ...
            <div>
              <h2 className="text-3xl font-bold text-astrian-charcoal mb-8 font-display">Credentials & Badges</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {instructor.achievements.map((ach, idx) => (
                  <Card key={idx} className="p-6 bg-white border border-astrian-clay/60 rounded-[2rem] flex gap-4 hover:border-astrian-sage/20 transition-all duration-300">
                    <div className="h-10 w-10 shrink-0 rounded-xl bg-astrian-sage/10 text-astrian-sage flex items-center justify-center">
                      <ShieldCheck className="h-5 w-5" />
                    </div>
                    <div>
                      <h4 className="font-bold text-base text-astrian-charcoal mb-1">{ach.title}</h4>
                      <p className="text-xs text-astrian-charcoal/60 leading-relaxed font-light">{ach.desc}</p>
                    </div>
                  </Card>
                ))}
              </div>
          ...
          <section className="mb-20">
            <h2 className="text-3xl font-bold text-astrian-charcoal mb-8 font-display text-center md:text-left">Weekly Studio Availability</h2>
                ...
                return (
                  <div 
                    ...
                    className={`p-6 rounded-[2rem] border text-center flex flex-col justify-between h-[150px] transition-all duration-300 ${
                      isRest 
                        ? "bg-astrian-cream/40 border-astrian-clay/40 text-astrian-charcoal/40" 
                        : "bg-white border-astrian-clay/80 text-astrian-charcoal shadow-sm hover:border-astrian-sage/30 hover:shadow-md"
                    }`}
                  >
                    <span className="text-sm font-semibold uppercase tracking-wider text-astrian-charcoal/50">{sched.day.substring(0, 3)}</span>
                    <div className="my-2 flex justify-center text-astrian-sage">
                      {!isRest && <Clock className="h-5 w-5 opacity-70" />}
                    </div>
                    <span className={`text-xs font-semibold ${isRest ? "italic text-astrian-charcoal/40" : "text-astrian-charcoal"}`}>
                      {sched.time}
                    </span>
                  </div>
                );
          ...
          <section className="max-w-4xl mx-auto">
            <h2 className="text-3xl font-bold text-astrian-charcoal mb-8 font-display text-center">Student Voices</h2>
            <div className="relative bg-white rounded-[3rem] border border-astrian-clay/80 p-8 md:p-16 shadow-[0_16px_40px_rgba(17,24,39,0.03)] text-center overflow-hidden">
              <div className="absolute top-6 left-6 text-6xl text-astrian-clay select-none font-serif">“</div>
              ...
              <div className="min-h-[140px] flex items-center justify-center">
                <motion.div
                  ...
                  className="space-y-6"
                >
                  <p className="text-xl md:text-2xl text-astrian-charcoal/80 leading-relaxed font-light italic text-pretty">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-astrian-charcoal">
              ...
              <div className="flex justify-center gap-4 mt-8">
                <button 
                  ...
                  className="h-10 w-10 rounded-full border border-astrian-clay hover:border-astrian-sage hover:bg-astrian-sage hover:text-white flex items-center justify-center transition-all duration-300 cursor-pointer"
